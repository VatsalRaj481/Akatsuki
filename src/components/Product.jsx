...
const ProductPage = () => {
  ...
  // --- MODIFICATION START ---
  // Add stock-related fields to productData state
  ...
  const handleInputChange = (e) => {
    ...
    } else if (e.target.name === "initialStockQuantity" || e.target.name === "reorderLevel") {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      initialStockQuantity: productData.initialStockQuantity !== "" ? productData.initialStockQuantity : null, // Send null if empty/not provided
      reorderLevel: productData.reorderLevel !== "" ? productData.reorderLevel : null, // Send null if empty/not provided
    };
    ...
    try {
      if (editProduct) {
        ...
        // Add new product
        await axios.post("http://localhost:8080/api/products", payload, { // Use the payload
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const deleteProduct = async (id) => {
    if (!window.confirm("Are you sure you want to delete this product? This will also remove its stock record.")) {
    ...
    try {
      ...
      showNotification("Product and associated stock deleted successfully!", "success");
    } catch (error) {
  ...
  const startEditProduct = (product) => {
    ...
    setProductData({
      ...
      initialStockQuantity: product.stockDetails ? product.stockDetails.quantity : "",
      reorderLevel: product.stockDetails ? product.stockDetails.reorderLevel : "",
    });
    // --- MODIFICATION END ---
  ...
  const openAddProductForm = () => {
    ...
    setProductData({ name: "", price: "", description: "", imageUrl: "", initialStockQuantity: "", reorderLevel: "" });
    // --- MODIFICATION END ---
        ...
        <div className="grid grid-cols-4 gap-6 mt-2 max-h-none px-4 justify-start">
          {filteredProducts.map((product) => (
            ...
            >
              ...
              {/* Stock Display - Check stockDetails before accessing properties */}
              <p
              ...
              >
                {product.stockDetails
                  ? product.stockDetails.quantity > 0
                    ...
                    : "Out of Stock"
                  : "Stock: N/A"} {/* Added N/A if stockDetails is null/undefined */}
              </p>
              {/* --- NEW: Display Reorder Level --- */}
              {product.stockDetails && product.stockDetails.reorderLevel !== undefined && (
            ...
            <div className="bg-gray-800 p-8 rounded-xl shadow-2xl w-full max-w-md mx-auto relative transform transition-all duration-300 ease-out scale-100 opacity-100">
              ...
              <div className="space-y-4">
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className="w-full p-3 bg-gray-700 rounded-lg border border-gray-600 focus:ring-2 focus:ring-green-500 focus:border-transparent text-white placeholder-gray-400"
                  min="0" // Ensure non-negative reorder level
                />
                {/* --- MODIFICATION END --- */}
              </div>
              <button
